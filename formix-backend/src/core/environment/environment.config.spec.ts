describe('EnvironmentConfig', () => {
  ...
  it('should pass validation with all required vars', () => {
    expect(() => {
      const { validateConfig } = require('./environment.config');
      validateConfig({ MONGODB_URI: 'mongodb://localhost:27017/test', PORT: '3001', NODE_ENV: 'test' });
    }).not.toThrow();
  });

  it('should use default PORT when not provided', () => {
    const { validateConfig } = require('./environment.config');
    const result = validateConfig({ MONGODB_URI: 'mongodb://localhost:27017/test', NODE_ENV: 'test' });
